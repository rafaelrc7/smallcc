...
instance Assembly Program where
  emit :: Program -> Text
  emit (Program func) = emit func `T.append` "\n.section .note.GNU-stack,\"\",@progbits\n\n"
...
instance Assembly FunctionDefinition where
  ...
  emit func = ".globl " `T.append` name `T.append` "\n"
    ...
    where name = funcName func
          instructions = T.concat $ map emit $ funcInstructions func

instance Assembly Instruction where
  emit :: Instruction -> Text
  emit Ret = "ret\n"
  emit Mov {movSrc=src, movDst=dst} = "movl " `T.append` emit src `T.append` ", " `T.append` emit dst `T.append` "\n"
