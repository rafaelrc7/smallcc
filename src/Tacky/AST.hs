...
newLabel caption =
  gets envLastLabel >>= \lastLabel ->
    gets envFuncName >>= \funcName ->
...
instance TackyGenerator (P.UnlabeledStatement TypeCheckingPhase) () where
  ...
  translate (P.Switch (label, labelsMap) expr body) =
    ...
    where
      translateLabel :: Val -> (SwitchLabel, Identifier) -> TackyGenerationMonad ()
      translateLabel _ (SDefault, _) = pure ()
      translateLabel exprVal (SCase caseVal, caseLabel) =
        do
          caseVal' <- translate caseVal
          test <- newTmpVar
          tell
            [ Binary {binaryOperator = EqualsTo, binarySrcs = (exprVal, caseVal'), binaryDst = test},
...
translateAssignmentExp op lhs rhs =
  do
    ...
    tmp <- newTmpVar
    tell
      [ Binary {binaryOperator = op, binarySrcs = (lhs', rhs'), binaryDst = tmp},
