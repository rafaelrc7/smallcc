...
replacePseudoRegisters function = (function', lastOffsetVarMap varMap''')
  where
    (function', varMap''') = replaceInFunction newVarMap function
    ...
    replaceInFunction varMap (Function name instructions) = (Function name instructions', varMap')
      where
        (instructions', varMap') =
          foldr
            (\i (is, vm) -> let (i', vm') = replaceInInstruction vm i in (i' : is, vm'))
            ([], varMap)
            instructions
    ...
    replaceInInstruction varMap Mov {movSrc = src, movDst = dst} = (Mov {movSrc = src', movDst = dst'}, varMap'')
      where
        (src', varMap') = replaceInOperand varMap src
    ...
    replaceInInstruction varMap Unary {unaryOp = op, unaryOperand = operand} = (Unary {unaryOp = op, unaryOperand = operand'}, varMap')
      where
        (operand', varMap') = replaceInOperand varMap operand
    replaceInInstruction varMap Binary {binaryOp = op, binaryOperands = (operandl, operandr)} = (Binary {binaryOp = op, binaryOperands = operands}, varMap'')
      where
        (operandl', varMap') = replaceInOperand varMap operandl
    ...
    replaceInOperand varMap (Pseudo identifier) = (Stack offset, varMap')
      where
        (varMap', offset) = fetchVarMap varMap identifier
    replaceInOperand varMap operand = (operand, varMap)
...
fixInstructions (function, lastOffset) = fixFunction function
  where
    ...
    fixFunction (Function name instructions) = Function name instructions'
      where
        instructions' = AllocateStack (roundToMultipleOf16 lastOffset) : concatMap fixInstruction instructions

    fixInstruction :: Instruction -> [Instruction]
    fixInstruction Mov {movSrc = src@(Stack _), movDst = dst@(Stack _)} =
      [ Mov {movSrc = src, movDst = Reg R10},
        Mov {movSrc = Reg R10, movDst = dst}
      ]
    fixInstruction (Idiv imm@(Imm _)) =
      [ Mov {movSrc = imm, movDst = Reg R10},
        Idiv (Reg R10)
      ]
    fixInstruction Binary {binaryOp = op@Mult, binaryOperands = (operandl, operandr@(Stack _))} =
      [ Mov {movSrc = operandr, movDst = Reg R11},
        Binary {binaryOp = op, binaryOperands = (operandl, Reg R11)},
        Mov {movSrc = Reg R11, movDst = operandr}
      ]
    fixInstruction Binary {binaryOp = op@ShiftLeft, binaryOperands = (operandl@(Stack _), operandr)} =
      [ Mov {movSrc = operandl, movDst = Reg CX},
        Binary {binaryOp = op, binaryOperands = (Reg CX, operandr)}
      ]
    fixInstruction Binary {binaryOp = op@ShiftRight, binaryOperands = (operandl@(Stack _), operandr)} =
      [ Mov {movSrc = operandl, movDst = Reg CX},
        Binary {binaryOp = op, binaryOperands = (Reg CX, operandr)}
      ]
    fixInstruction Binary {binaryOp = op, binaryOperands = (operandl@(Stack _), operandr@(Stack _))} =
      [ Mov {movSrc = operandl, movDst = Reg R10},
        Binary {binaryOp = op, binaryOperands = (Reg R10, operandr)}
      ]
    fixInstruction (Cmp op1@(Stack _) op2@(Stack _)) =
      [ Mov {movSrc = op1, movDst = Reg R10},
        Cmp (Reg R10) op2
      ]
    fixInstruction (Cmp op1 op2@(Imm _)) =
      [ Mov {movSrc = op2, movDst = Reg R11},
        Cmp op1 (Reg R11)
      ]
    fixInstruction i = [i]
...
translateFunctionDefinition (T.Function name params body) =
  ...
  where
    (regParams', stackParams') = splitAt 6 params
...
translateStackParameters ((param, paramN) : params) = Mov {movSrc = Stack stackOffset, movDst = Pseudo param} : translateStackParameters params
  where
    stackOffset = fromInteger $ toInteger $ 16 + 8 * paramN

translateInstruction :: [T.Instruction] -> [Instruction]
translateInstruction [] = []
translateInstruction (T.Return expr : is) =
  [ Mov {movSrc = translateOperand expr, movDst = Reg AX},
    Ret
  ]
...
translateInstruction (T.FunCall name args ret : is) =
  stackPaddingIns
    ...
    ++ translateInstruction is
  where
    (regArgs', stackArgs') = splitAt 6 args
