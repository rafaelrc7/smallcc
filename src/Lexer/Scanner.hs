...
scanToken =
  peekSymbol >>= \case
    ...
    Just s
      | isSpace s -> consumeSymbol >> nextToken
...
scanLineMarker = do
  (LexerState (RemainingBuffer _ (Location _ c _)) _) <- get
  expect '#'
...
consumeSymbol =
  get >>= \(LexerState (RemainingBuffer b bloc) (CurrentLexeme l lloc)) ->
    case L.uncons b of
      Nothing -> return Nothing
      Just (s@'\n', b') ->
        do
          put (LexerState (RemainingBuffer b' (nextLine bloc)) (CurrentLexeme (l `T.snoc` s) lloc))
          return $ Just s
      Just (s, b') ->
        do
          put (LexerState (RemainingBuffer b' (nextCol bloc)) (CurrentLexeme (l `T.snoc` s) lloc))
          return $ Just s

peekSymbol :: LexerMonad (Maybe Char)
peekSymbol =
  get >>= \(LexerState (RemainingBuffer b _) _) ->
    case L.uncons b of
...
scanConstant =
  ...
  where
    token :: LexerMonad Token
    token =
      get >>= \(LexerState (RemainingBuffer _ _) (CurrentLexeme l _)) ->
        case T.decimal l of
...
scanIdentifier =
  peekSymbol >>= \case
    Nothing -> token
    Just s
      | isAlphaNum_ s -> consumeSymbol >> scanIdentifier
      | otherwise -> token
  where
    token :: LexerMonad Token
    token =
      get >>= \(LexerState _ (CurrentLexeme l _)) ->
...
scanCompoundToken tokens =
  peekSymbol >>= \case
    ...
    Just s -> catchST token $ consumeSymbol >> scanCompoundToken tokens'
      where
        scanCompoundToken' :: [(String, TokenType)] -> (Maybe TokenType, [(String, TokenType)])
