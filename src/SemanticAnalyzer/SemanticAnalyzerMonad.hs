...
data Environment = Environment
  { envFunctionName      :: Maybe Identifier,
...
resolveNewIdentifier hasLinkage name =
  do
    currentScope <- gets envCurrentScope
...
getIdentifier identifier = do
  currentScopeVarEnv <- gets envCurrentScope
...
resolveLabel label =
  do
    functionName <- gets envFunctionName
    labelCounter <- gets envIdentifierCounter
    labelEnv <- gets envLabelEnv
    let label' = maybe "" (<> ".") functionName <> label
    if label' `M.member` labelEnv
      then throwError $ DuplicateLabelDeclaration label
      else do
...
withEnclosingLabel label action =
  do
    enclosingLabels <- gets envEnclosingLabels
    localState
...
getEnclosingLoopLabel = gets envEnclosingLabels >>= getEnclosingLoopLabel'
  where
    getEnclosingLoopLabel' :: [EnclosingLabel] -> SemanticAnalyzerMonad Identifier
...
getEnclosingSwitchLabel = gets envEnclosingLabels >>= getEnclosingSwitchLabel'
  where
    getEnclosingSwitchLabel' :: [EnclosingLabel] -> SemanticAnalyzerMonad Identifier
...
addSymbolType name typ =
  do
    symbolTypes <- gets envSymbolTypes
...
instance IdentifierResolver FunctionDeclaration where
  ...
  resolveIdentifiers (FunctionDeclaration _ name params body) =
    do
      name' <- resolveNewIdentifier True name
      ...
      fun <- withNewVarScope $ do
        ...
        body' <- case body of
          ...
          Just (Block body') ->
            if isNothing currentName
              then Just <$> (Block <$> mapM resolveIdentifiers body')
  ...
  resolveSwitch (Case label expr) =
    do
      ...
      case switchLabels of
        Nothing -> throwError StatementOutsideSwitch
        Just switchLabels' ->
          let key = SCase constExpr
           in if key `M.member` switchLabels'
                then throwError DuplicateCaseValue
                else modify (\env -> env {envSwitchLabels = Just $ M.insert (SCase constExpr) label switchLabels'})
  ...
  resolveSwitch (Default label) =
    do
      ...
      case switchLabels of
        Nothing -> throwError StatementOutsideSwitch
        Just switchLabels' ->
          if SDefault `M.member` switchLabels'
            then throwError DuplicateDefaultValue
            else modify (\env -> env {envSwitchLabels = Just $ M.insert SDefault label switchLabels'})
  ...
  resolveTypes (VarDeclaration () name initialisation) =
    do
      ...
      initialisation' <- case initialisation of
        Nothing -> return Nothing
        Just ini' -> do
          initialisation' <- resolveTypes ini'
...
instance TypeChecker FunctionDeclaration where
  ...
  resolveTypes (FunctionDeclaration () name params body) =
    -- FunctionDeclaration (Fun (map (const Int) params) Int) name params <$> (resolveTypes <$?> body)
    do
      symbolTypes <- gets envSymbolTypes
      ...
      case M.lookup name symbolTypes of
        Just funType' -> do
          when (funType' /= funType) $ throwError IncompatibleTypes
      ...
      body' <- case body of
        Just b -> do
          mapM_ (`addSymbolType` Int) params
